#!/bin/bash
# This script checked on CM5.7.0

# Properties
CMNODE="node0.x.y"
TARGET="node0.x.y node1.x.y node2.x.y"
CLUSTER="Cluster1"
ROOT_PASS="root password"
BASE=http://$CMNODE:7180/api/v12
CDH_VER=5
...
rman_pass=`ssh -l root $CMNODE grep com.cloudera.cmf.REPORTSMANAGER.db.password /etc/cloudera-scm-server/db.mgmt.properties | cut -d= -f2`
### Configuration
curl -X PUT -u "admin:admin" -i \
  -H "content-type:application/json" \
